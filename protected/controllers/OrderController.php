...
// 		$_POST = array('status'=>100, 'refnumber'=>'764123452', 'resnumber'=>'1206959891');
		if(isset($_POST['status'])) {
			$orderService = new OrderService();
			$paymentModel = $orderService->getPayment($_POST['order_id']);
		
			if($_POST['status'] == 100 AND $paymentModel != null) {
				$receive = $orderService->receiveRestFromParsPal($_POST, $paymentModel->price); //$paymentModel->price : Rial
				
				if($receive['status'] == 'SUCCESSFUL') {
