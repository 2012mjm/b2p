...
class JDateTime
{
    ...
    {
        if ( $this->jalali  !== null ) $this->jalali = ($this->jalali === false) ? false : true;
        if ( $this->convert  !== null ) $this->convert = ($this->convert === false) ? false : true;
        if ( $this->timezone !== null ) $this->timezone = ($this->timezone != null) ? $this->timezone : null;
    }
    /**
     * jDateTime::Date
     *
   ...
   /**
    ...
    /**
     * Jalali to Gregorian Conversion
     * Copyright (C) 2016 Mohammad Javad Masoudian
     * Input Pattern: Y-m-d H:i:s || Y-m-d
     *
     */
    public function toGregorianOnePart($dateTime)
    {
    	list($date, $time) = explode(' ', $dateTime);
    	list($y, $m, $d) = explode('-', $date);
    	list($gy, $gm, $gd) = $this->toGregorian($y, $m, $d);
    	$dateFinal = $gy.'-'.$gm.'-'.$gd;
